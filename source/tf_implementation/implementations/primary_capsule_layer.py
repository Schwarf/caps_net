...
class PrimaryCapsuleLayer(IPrimaryCapsuleLayer, object):
    def __init__(self, dimension_of_capsule, number_of_channels, kernel_size, activation_function, stride_size, padding):
        ...
        if stride_size is None or stride_size < 1:
            raise ValueError(f"The stride_size must be larger than zero but is '{stride_size}'.")
